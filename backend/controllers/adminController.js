...
const getAllProduct = async (req, res) => {
  ...
  try {
    ...
    if (!products || products.length === 0) {
      return res
        .status(404)
        .json({ error: true, message: "No Product Found." });
    }
      ...
      products.map(async (product) => {
        ...
        if (productData.imageName) {
          try {
            ...
            console.error(
              `Error generating signed URL for ${productData.imageName}`,
              err,
            );
            return {
              ...productData,
...
const addProduct = async (req, res) => {
  ...
  if (!quantity)
    return res
      .status(400)
      .json({ error: true, message: "Quantity is required." });
  ...
  try {
    ...
    if (!category) {
      return res
        .status(404)
        .json({ error: true, message: `Category ${category} not found.` });
    }
  ...
  try {
    ...
    if (file) {
      ...
      if (
        ...
        existingProduct.imageName !== file.originalname
      ) {
        const deleteParams = {
          Bucket: bucketName,
